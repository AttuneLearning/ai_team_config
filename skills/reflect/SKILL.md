...
5. **Auto-Actions**
   - Increment `usage_count` on patterns used (in `memory/patterns/`)
   - Flag patterns with 5+ uses for promotion review
   - If new pattern created, save to `memory/patterns/`
   - If ADR suggestion created, save to `dev_communication/shared/architecture/suggestions/`
