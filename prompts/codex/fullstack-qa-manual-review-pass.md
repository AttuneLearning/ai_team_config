# Fullstack-QA Manual Review Pass
...
1. Read every file in `dev_communication/fullstack/inbox/` (not `completed/`) and `dev_communication/fullstack/issues/active/`
...
4. For Soundsafe-class projects, invoke the matching specialized review subagent (`.claude/agents/dsp-reviewer`, `safety-reviewer`, `crypto-reviewer`, `platform-boundary-reviewer`, or `accessibility-reviewer`) for the touched domain
5. Map acceptance criteria to the actual test evidence
...
If the prior automated gate evidence is stale or incomplete, stop manual review for that issue and route it back to the full `fullstack-qa-autonomous-sweep`. After each issue, rescan and continue. End with summary: passed, blocked, remaining.
